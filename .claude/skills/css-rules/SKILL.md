...
- 「フルハイト」「100%」「画面いっぱい」等の指示時は **svh / lvh の選択を確認**
- **ファーストビュー（ヒーロー）のデフォルト**: `95lvh`
  - svh はアドレスバー縮小時の高さ → 下に隙間ができやすい
  - lvh はアドレスバー表示時の高さ → 95% で収めると安定
- デフォルト適用時は「プロジェクトルールにより95lvhを適用しました」と報告
